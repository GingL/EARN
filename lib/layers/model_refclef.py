...
from layers.vis_enc import SubjectEncoder
...
class KARN(nn.Module):
    def __init__(self, opt):
        ...
        self.feat_fuse = nn.Sequential(
            nn.Linear(self.fc7_dim + self.pool5_dim + 5 + self.fc7_dim + 5, opt['jemb_dim']))


    def forward(self, pool5, fc7, lfeats, cxt_fc7, cxt_lfeats, dist, labels, enc_labels, dec_labels,
        ...
        rel_attn, rel_phrase_emb = self.rel_attn(context, embedded, labels) # (sent_num, 10), (sent_num, 512)
        ...
        # object attention
        cxt_fc7_att = cxt_fc7.view(sent_num, ann_num, self.fc7_dim, -1).mean(3)
        ...
        # object feats
        rel_feats, dist = self.rel_encoder(cxt_fc7_att, cxt_lfeats, obj_attn, wo_obj_idx,
