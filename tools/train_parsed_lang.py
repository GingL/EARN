...
def eval_split(loader, model, split, opt):
    ...
    while True:
        ...
        sim = data['sim']
        parsed_lang = data['parsed_lang']
        parsed_subs = parsed_lang['subjects']
        parsed_locs = parsed_lang['locations']
        parsed_cxts = parsed_lang['contexts']
        ...
        for i, sent_id in enumerate(sent_ids):
            ...
            parsed_sub = parsed_subs[i:i+1]
            max_len_sub = (parsed_sub != 0).sum().data[0]
            parsed_sub = parsed_sub[:, :max_len_sub]

            parsed_loc = parsed_locs[i:i+1]
            max_len_loc = (parsed_loc != 0).sum().data[0]
            parsed_loc = parsed_loc[:, :max_len_loc]

            parsed_cxt = parsed_cxts[i:i+1]
            max_len_cxt = (parsed_cxt != 0).sum().data[0]
            parsed_cxt = parsed_cxt[:, :max_len_cxt]
            ...
            # if i ==2:
            #     print('2')
            scores, loss, sub_idx, sub_attn, obj_attn, weights, sub_ann_attn, loc_ann_attn, rel_ann_attn = model(pool5, fc7, lfeats, dif_lfeats, cxt_fc7,
                                 cxt_lfeats, dist, label, enc_label, dec_label, sub_sim, obj_sim,
                                 sub_emb, obj_emb, att_label, select_ix, att_weights, parsed_sub, parsed_loc, parsed_cxt)
...
class KARN(nn.Module):
    def __init__(self, opt):
        ...
        self.fc_parsed_lang = nn.Linear(self.word_vec_size, self.vocab_size)
        self.cross_entropy = nn.CrossEntropyLoss(reduce=False)

    def forward(self, pool5, fc7, lfeats, dif_lfeats, cxt_fc7, cxt_lfeats, dist, labels, enc_labels, dec_labels,
                sub_sim, obj_sim, sub_emb, obj_emb, att_labels, select_ixs, att_weights, parsed_sub, parsed_loc, parsed_cxt):
        ...
        _, _, sub_phrase_emb = self.rnn_encoder(parsed_sub)
        _, _, loc_phrase_emb = self.rnn_encoder(parsed_loc)
        _, _, rel_phrase_emb = self.rnn_encoder(parsed_cxt)
        sub_phrase_emb, _ = sub_phrase_emb.max(1)
        loc_phrase_emb, _ = loc_phrase_emb.max(1)
        rel_phrase_emb, _ = rel_phrase_emb.max(1)
        ...
        # sub_phrase = self.fc_parsed_lang(sub_phrase_emb)
        # loc_phrase = self.fc_parsed_lang(loc_phrase_emb)
        # rel_phrase = self.fc_parsed_lang(rel_phrase_emb)
        ...
        return total_ann_score, loss, sub_idx, sub_attn, obj_attn, weights, sub_ann_attn, loc_ann_attn, rel_ann_attn


class Loader(object):

    def __init__(self, data_json, sub_obj_wds, similarity, parsed_language, data_h5=None, data_emb=None):
        print('Loader loading parsed_language file:', parsed_language)
        self.parsed_language_info = torch.load(parsed_language)
        self.parsed_language = {sent_id: wds for sent_id, wds in self.parsed_language_info.items()}
    ...
    def fetch_parsed_lang(self, sent_id):
        parsed_lang = self.parsed_language[sent_id]
        max_length  = 10
        subject = np.zeros((1, max_length), dtype=np.int32)
        location = np.zeros((1, max_length), dtype=np.int32)
        context = np.zeros((1, max_length), dtype=np.int32)
        parsed_sub = parsed_lang[0] if len(parsed_lang[0])<=10 else parsed_lang[0][0:10]
        parsed_loc = parsed_lang[1] if len(parsed_lang[0])<=10 else parsed_lang[0][0:10]
        parsed_cxt = parsed_lang[2] if len(parsed_lang[0])<=10 else parsed_lang[0][0:10]

        for i, wd_id in enumerate(parsed_sub):
            subject[0, i] = wd_id
        for i, wd_id in enumerate(parsed_loc):
            location[0, i] = wd_id
        for i, wd_id in enumerate(parsed_cxt):
            context[0, i] = wd_id      
        return subject, location, context
...
class DataLoader(Loader):
    ...
    def __init__(self, data_json, data_h5, sub_obj_wds, similarity, parsed_language,opt):
        # parent loader instance
        Loader.__init__(self, data_json, sub_obj_wds, similarity, parsed_language, data_h5)
        # prepare attributes
    ...
    def getBatch(self, split, opt):
        ...
        subjects, locations, contexts = [],[],[]
        for sent_id in img_sent_ids:
            subject, location, context = self.fetch_parsed_lang(sent_id)
            subjects.append(subject)
            locations.append(location)
            contexts.append(context)
        subjects = np.vstack(subjects)
        subjects = Variable(torch.from_numpy(subjects).long().cuda())
        max_len_sub = (subjects!=0).sum(1).max()
        subjects = subjects[:, :max_len_sub]

        locations = np.vstack(locations)
        locations = Variable(torch.from_numpy(locations).long().cuda())
        max_len_loc = (locations!=0).sum(1).max()
        locations = locations[:, :max_len_loc]

        contexts = np.vstack(contexts)
        contexts = Variable(torch.from_numpy(contexts).long().cuda())
        max_len_cxt = (contexts!=0).sum(1).max()
        contexts = contexts[:, :max_len_cxt]
        ...
        data['parsed_lang'] = {'subjects': subjects, 'locations': locations, 'contexts': contexts}
    ...
    def getTestBatch(self, split, opt):
        ...
        subjects = np.vstack(subjects)
        subjects = Variable(torch.from_numpy(subjects).long().cuda())
        max_len_sub = (subjects!=0).sum(1).max().data[0]
        subjects = subjects[:, :max_len_sub]

        locations = np.vstack(locations)
        locations = Variable(torch.from_numpy(locations).long().cuda())
        max_len_loc = (locations!=0).sum(1).max().data[0]
        locations = locations[:, :max_len_loc]

        contexts = np.vstack(contexts)
        contexts = Variable(torch.from_numpy(contexts).long().cuda())
        max_len_cxt = (contexts!=0).sum(1).max().data[0]
        contexts = contexts[:, :max_len_cxt]
...
def main(args):
    ...
    # set up loader
    parsed_language = osp.join(RootPath, 'cache/prepro', opt['dataset_splitBy'], 'parsed_language.pt')
    ...
    loader = DataLoader(data_h5=data_h5, data_json=data_json, sub_obj_wds=sub_obj_wds, similarity=similarity, parsed_language=parsed_language,opt=opt)
    ...
    while True:
        ...
        att_labels, select_ixs = data['att_labels'], data['select_ixs']
        parsed_lang = data['parsed_lang']
        parsed_sub = parsed_lang['subjects']
        parsed_loc = parsed_lang['locations']
        parsed_cxt = parsed_lang['contexts']

        T['data'] = time.time() - tic

        tic = time.time()
        scores, loss, _, _, _, _, _, _, _= model(Feats['pool5'], Feats['fc7'], Feats['lfeats'], Feats['dif_lfeats'], Feats['cxt_fc7'],
                             Feats['cxt_lfeats'],Feats['dist'], labels, enc_labels, dec_labels, sub_sim, obj_sim, sub_emb, obj_emb, att_labels, 
                             select_ixs, att_weights, parsed_sub, parsed_loc, parsed_cxt)
